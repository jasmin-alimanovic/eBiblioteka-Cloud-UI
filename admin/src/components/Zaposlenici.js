...
import { getZaposlenike } from "../services/zaposlenikService";
import AddZaposlenikModal from "./modals/AddZaposlenikModal";
import EditZaposlenikModal from "./modals/EditZaposlenikModal";

//
export default function Zaposlenici() {
  ...
  useEffect(() => {
    document.title = "eBiblioteka Cloud-Zaposlenici";
  }, []);
  useEffect(() => {
    getZaposlenike(sort, query, page, pageSize).then((data) => {
      setUsers(data);
    });
                ...
                {users !== null && users.data.length > 0 ? (
                  users.data.map((user) => (
                    <tr className="book-row" key={user.id}>
                      ...
                      <td className="p-2">{user.uloga.naziv}</td>
